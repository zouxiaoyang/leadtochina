...
/*2011-5-16
		if ($row->prev) {
			...
			$row->prev = JRoute::_(ContentHelperRoute::getArticleRoute($row->prev->slug, $row->prev->catslug,0,$row->prev->alias));
		} else {
			$row->prev = '';
		}

		if ($row->next) {
			$row->next = JRoute::_(ContentHelperRoute::getArticleRoute($row->next->slug, $row->next->catslug,0,$row->next->alias));
		} else {
			$row->next = '';
		}
...
2011-5-16*/
		...
		if($row->prev){
			$row->prev="/news/".$row->prev->alias.".html";
		}

		if($row->next){
			$row->next="/news/".$row->next->alias.".html";
		}
		// output
