...
class modMenuHelper
{
	...
	function buildMenu()
	{
		...
		$usertype	= $_SESSION['__default']['user']->usertype;
		...
		$editAllComponents	= 1;//$user->authorize('com_components', 'manage');
		$canMassMail		= $user->authorize('com_massmail', 'manage');
		$canManageUsers		= $user->authorize('com_users', 'manage');

		// Menu Types
		require_once( JPATH_ADMINISTRATOR.DS.'components'.DS.'com_menus'.DS.'helpers'.DS.'helper.php' );
		$menuTypes 	= MenusHelper::getMenuTypelist();///home/fl/work/leadtochina/administrator/components/com_menus/helpers/helper.php
	...
	if($usertype!='销售'){
		...
		if ($editAllComponents)
		{
			$menu->addChild(new JMenuNode(JText::_('Components')), true);
			 $where= '';
			if($usertype=='销售'){ $where = ' and id in (109,66)';}elseif($usertype=='Manager'){ $where = ' and id not in (109,66)'; }
			$query = 'SELECT *' .
				' FROM #__components' .
				' WHERE '.$db->NameQuote( 'option' ) .' <> "com_frontpage"' .
				' AND '.$db->NameQuote( 'option' ).' <> "com_media"' .
				' AND enabled = 1' . $where .
				' ORDER BY ordering, name';//echo $query;exit;
			$db->setQuery($query);
			...
			foreach ($comps as $row)
			{
				if ($editAllComponents | $user->authorize('administration', '销售', 'components', $row->option))
				{
