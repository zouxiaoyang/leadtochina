 ...
 * @subpackage	Config
 */
class TotalorderController extends JController
{
	/**
	 * Custom Constructor
	 */
	...
	function remove()
	{
		...
		$this->setRedirect( 'index.php?option=com_totalorder', $msg );
	}
...
*/
	...
	function updateexcutor(){
		...
		if(empty($executor)){
			...
			$this->setError(JText::_('名字不能够为空'));
			exit;		
		
		}
		$db->setQuery("UPDATE jos_total_order SET executor='$executor' WHERE id=$id");
		$db->query();
		echo "设置成功！";
		exit;
	...
	{
		JRequest::checkToken() or jexit( 'Invalid Token' );
		$this->setRedirect( 'index.php?option=com_totalorder' );
	}
}