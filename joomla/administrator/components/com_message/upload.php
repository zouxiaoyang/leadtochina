 ...
 * @subpackage	Config
 */
class ProvinceControllerUpload extends JController
{
	/**
	 * Custom Constructor
	 */
...
//["textfieldcheck"]=> string(0) "" ["c293f03092a989911df312e31cbf88f2"]=> string(1) "1" }
		...
		if($_FILES['ufile']['size']){
			...
			$thum=new resizeimage(JPATH_SITE.DS."uploads".DS."province".DS.$return_obj->_fileList['name'],160,160,0,JPATH_SITE.DS."uploads".DS."province".DS."thumb".DS."thumb_".$return_obj->_fileList['name']);
			$row->pic_dir=null;
			$row->pic_path=null;
		...
		switch ($this->_task)
		{
			case 'apply':
				...
				$link = 'index.php?option=com_province&view=cntours&task=edit&cid[]='. $row->provinceid;
				break;

			case 'save':
			default:
				$msg = JText::_( 'saved' );
				$link = "index.php?option=com_province&c=upload&view=uploadpic&task=upload&cid[]=". $post['provinceid'];
				break;
		}
