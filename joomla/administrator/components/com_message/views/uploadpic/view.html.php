 ...
 * @since 1.0
 */
class ProvinceViewUploadpic extends JView
{
...
*/
		$db->setQuery("SELECT id,pic_dir ,pic_path FROM jos_cos_provincemap  WHERE provinceid=$uid ORDER BY id DESC");
		//$pro_res=$db->query();
		...
		for($i=1;$i<=$pic_count;$i++){
			...
			$pic_str.="<td><img src=\"$temp_file_path\"><br /><a href=\"#\" onclick=\"javascript:del('../index.php?option=com_delpic&from=province&id={$piclist[$temp_index]->id}')\">删除</a></td>";
