 ...
 * @subpackage	Config
 */
class ChinacultureControllerUpload extends JController
{
	/**
	 * Custom Constructor
	 */
...
//["textfieldcheck"]=> string(0) "" ["c293f03092a989911df312e31cbf88f2"]=> string(1) "1" }
		...
		if($_FILES['ufile']['size']){
			...
			$fupload=new Upload(JPATH_ROOT.DS."uploads".DS."chinaculture","chcul","com_chinaculture");
			...
			$thum=new resizeimage(JPATH_SITE.DS."uploads".DS."chinaculture".DS.$return_obj->_fileList['name'],140,140,0,JPATH_SITE.DS."uploads".DS."chinaculture".DS."thumb".DS."thumb_".$return_obj->_fileList['name']);

			$thum=new resizeimage(JPATH_SITE.DS."uploads".DS."chinaculture".DS.$return_obj->_fileList['name'],500,315,1,JPATH_SITE.DS."uploads".DS."chinaculture".DS.$return_obj->_fileList['name']);
			$row->pic_dir=null;
			$row->pic_path=null;
		...
		switch ($this->_task)
		{
			...
			default:
				$msg = JText::_( 'saved' );
				$link = "index.php?option=com_chinaculture&c=upload&view=uploadpic&task=upload&cid[]=". $post['chinaculid'];
				break;
		}
