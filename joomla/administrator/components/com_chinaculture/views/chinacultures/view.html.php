 ...
 * @since 1.0
 */
class ChinacultureViewChinacultures extends JView
{
	function display( $tpl = null )
	{
		...
		for($j=0;$j<$c;$j++){
			if(empty($rows[$j]->cat_path)==false){
				
				$cat_path_arr=explode(",",$rows[$j]->cat_path);
				$count=count($cat_path_arr);
				$cat_path_str="";
				...
				for($i=0;$i<$count;$i++){
					...
					$query = 'SELECT title'
					. ' FROM jos_categories  '
					. "WHERE id={$cat_path_arr[$i]}";
					$db->setQuery( $query);
					$cat_res=$db->query();
					$cat_row=mysql_fetch_array($cat_res);
					$cat_path_str.=$cat_row['title'].">>";
					
					mysql_fetch_array($cat_res);
					
				}
				$rows[$j]->cat_path_str=$cat_path_str;
				$cat_path_str="";
		...
		//$this->assignRef('cat_path_str',		$cat_path_str);
