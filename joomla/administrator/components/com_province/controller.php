 ...
 * @subpackage	Config
 */
class ProvinceController extends JController
{
	/**
	 * Custom Constructor
	 */
	...
	function display( )
	{
		switch($this->getTask())
		{
			...
			case 'edit'    :
			{
				...
				JRequest::setVar( 'view', 'province'  );//文件路径设置。是view
		...
		if(empty($view)) {
			JRequest::setVar('view', 'provinces');
		};
...
//["textfieldcheck"]=> string(0) "" ["c293f03092a989911df312e31cbf88f2"]=> string(1) "1" } 
	...
	function remove()
	{
		...
		{
			$poll =& JTable::getInstance('province', 'Table');
			if (!$poll->delete( $cid[$i] ))
			{
				$msg .= $poll->getError();
			}
		}
		$this->setRedirect( 'index.php?option=com_province', $msg );
	}
