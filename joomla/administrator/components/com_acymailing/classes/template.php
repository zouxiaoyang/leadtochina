...
class templateClass extends acymailingClass{
  ...
  function save($element){
      ...
      if(empty($element->namekey)) $element->namekey = time().JFilterOutput::stringURLSafe($element->name);
    	 ...
    	 if(file_exists(ACYMAILING_TEMPLATE.'css'.DS.'template_'.intval($element->tempid).'.css')){
    	 	...
    	 	if(!JFile::delete(ACYMAILING_TEMPLATE.'css'.DS.'template_'.intval($element->tempid).'.css')){
    	 		echo acymailing::display('Could not delete the file '.ACYMAILING_TEMPLATE.'css'.DS.'template_'.intval($element->tempid).'.css','error');
    	 	}
    	 }
    }
	...
	function buildCSS($style,$stylesheet){
		$inline = '';
		if(!empty($style)){
			foreach($style as $class => $style){
  ...
  function installtemplate($filepath){
		...
		if(!file_exists(ACYMAILING_TEMPLATE.$newTemplateFolder.DS.'index.html')){
			JFile::write(ACYMAILING_TEMPLATE.$newTemplateFolder.DS.'index.html','<html><body bgcolor="#FFFFFF"></body></html>');
		}
		$fileContent = preg_replace('#(href|src|action|background)[ ]*=[ ]*\"(?!(https?://|\#|mailto:|/))(?:\.\./|\./)?#','$1="components/com_acymailing/templates/'.$newTemplateFolder.'/',$fileContent);
    	...
    	foreach($allFiles as $oneFile){
    		if(preg_match('#(thumbnail|screenshot|muestra)#i',$oneFile)){
    			...
    			$newTemplate->description = '<img src="components/com_acymailing/templates/'.$newTemplateFolder.'/'.$oneFile.'" />';
    			break;
    		}
    	}
		static $order = 0;
