			...
			?>
				<tr class="<?php echo "row$k"; ?>" id="<?php echo $id; ?>">
					...
					<?php
						...
						echo acymailing::tooltip('ID : '.$row->mailid, $row->subject, 'tooltip.png', $row->subject,acymailing::completeLink($link));
					?>
					</td>
					<td align="center">
					<?php
						echo acymailing::tooltip('Name : '.$row->name.'<br/>ID : '.$row->subid, $row->email, 'tooltip.png', $row->email,acymailing::completeLink('subscriber&task=edit&cid[]='.$row->subid));
					?>
					</td>
					<td align="center">
