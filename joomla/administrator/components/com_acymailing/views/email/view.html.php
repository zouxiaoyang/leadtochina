...
class EmailViewEmail extends JView
{
	...
	function form(){
		...
		$script .= "var openTag = true; function displayTags(){var box=$('iframetag'); if(openTag){box.innerHTML = ".$iFrame."; box.setStyle('display','block');}";
		$script .= "var fx = box.effects({duration: 1500, transition: Fx.Transitions.Quart.easeOut});";
		$script .= "if(openTag){fx.start({'height': 300});}else{fx.start({'height': 0}).chain(function() {box.innerHTML = '';box.setStyle('display','none');})}; openTag = !openTag;}";
		...
		$script .= "var openTemplate = true; function displayTemplates(){var box=$('iframetemplate'); if(openTemplate){box.innerHTML = ".$iFrame."; box.setStyle('display','block');}";
		$script .= "var fx = box.effects({duration: 1500, transition: Fx.Transitions.Quart.easeOut});";
		$script .= "if(openTemplate){fx.start({'height': 300});}else{fx.start({'height': 0}).chain(function() {box.innerHTML = '';box.setStyle('display','none');})}; openTemplate = !openTemplate;}";
