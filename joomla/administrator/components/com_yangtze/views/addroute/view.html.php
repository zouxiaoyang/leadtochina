 ...
 * @since 1.0
 */
class YangtzeViewAddroute extends JView
{
...
*/
	...
	//循环获取住宿标准
	$hot_arr=array("Budget","Standard","Luxury");
	$hotel_str="";
	$hotel_sel="";
	for($j=0;$j<3;$j++){
		if(empty($options)==false and $options[0]->hotel_standard==$hot_arr[$j]){
			$hotel_sel="selected='selected'";
		}else{
			$hotel_sel="";
		}
		$hotel_str.="<option value=\"{$hot_arr[$j]}\" $hotel_sel>{$hot_arr[$j]}</option>";
