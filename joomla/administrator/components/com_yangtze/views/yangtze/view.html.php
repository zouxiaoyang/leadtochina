 ...
 * @since 1.0
 */
class YangtzeViewYangtze extends JView
{
...
*/
	...
	*/
	if(empty($options[0]->categorieid_str)==false){
		...
		if($category_id>0){
			$db->setQuery("SELECT parent_id FROM jos_categories WHERE id=$category_id AND section='com_yangtze' AND published=1");
			$sup_res=$db->query();
		...
		$db->setQuery("SELECT id,parent_id,title FROM jos_categories WHERE parent_id=0 AND section='com_yangtze' AND published=1");
		$cat_res=$db->query();
		$cat_seled="";
		$cat_str='';
		$blank="&nbsp;&nbsp;&nbsp;&nbsp;";
		...
		while($cat_row=mysql_fetch_assoc($cat_res)){
				...
				if($options[0]->categorieid_str==$cat_row['id'] OR $sup_id==$cat_row['id']){
