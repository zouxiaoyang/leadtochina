 ...
 * @subpackage	Config
 */
class YangtzeControllerRoute extends JController
{
	/**
	 * Custom Constructor
	 */
...
//["textfieldcheck"]=> string(0) "" ["c293f03092a989911df312e31cbf88f2"]=> string(1) "1" }
	...
	$post['package_tips'] = JRequest::getVar( 'package_tips', '', 'post', 'string', JREQUEST_ALLOWRAW );		
		...
		switch ($this->_task)
		{
			case 'apply':
				...
				$link = 'index.php?option=com_yangtze&view=yangtze&task=edit&cid[]='. $row->packageid;
				break;

			case 'save':
			default:
				$msg = JText::_( 'saved' );
				$link = 'index.php?option=com_yangtze&view=yangtze&c=route&task=editroute&cid[]='.$row->packageid;
				break;
		}
	...
	function remove()
	{
		...
		$this->setRedirect( "index.php?option=com_yangtze&view=yangtze&c=route&task=editroute&cid[]=$packageid", $msg );
	}
...
*/
	function cancel()
	{
		...
		$row->checkin( $id );
		$this->setRedirect( 'index.php?option=com_yangtze' );
	}
}