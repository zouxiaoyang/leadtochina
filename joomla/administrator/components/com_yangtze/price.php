 ...
 * @subpackage	Config
 */
class YangtzeControllerPrice extends JController
{
	/**
	 * Custom Constructor
	 */
		...
		if($post["package_class"]=="small"){
			...
			foreach ($gro_options as $i=>$code)
			{
				...
				if ($isNew=="yes")
				{
					...
					$db->setQuery("UPDATE jos_cos_tours_package SET price=$res_min WHERE id=$packageid");
					$db->query();
				}
				else
				{
...
*/
	...
	switch ($this->_task)
		{
			case 'apply':
				$msg = JText::_( 'Changes to  saved' );
					$link = 'index.php?option=com_yangtze&view=yangtze&c=price&task=editsmprice&cid[]='.$post['packageid'];
				break;
	...
	function remove()
	{
		...
		if($packclass=="small"){
		$this->setRedirect( "index.php?option=com_cyangtze&view=yangtze&c=price&task=editprice&cid[]=$packageid", $msg );}else{
		$this->setRedirect( "index.php?option=com_yangtze&view=yangtze&c=price&task=editsmprice&cid[]=$packageid", $msg );		
		}
	}
