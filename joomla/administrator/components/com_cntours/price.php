 ...
 * @subpackage	Config
 */
class CntoursControllerPrice extends JController
{
	/**
	 * Custom Constructor
	 */
	...
	{		
		...
		if($post["serialize"]==1){			
			...
			$obj->price_instruction=JRequest::getVar( 'price_instruction', '', 'post', 'string', JREQUEST_ALLOWRAW );
      if($post['price_serialize'][1]){
        ...
        $obj->price1 = $post["price_serialize_1"][8];
			...
			$this->setRedirect( "index.php?option=com_cntours&c=price&view=price&task=editprice&cid[]={$post['packageid']}", JText::_( 'saved' ) );
		...
		if($post["package_class"]=="small"){
			...
			//$price_options = JArrayHelper::getValue( $post, 'price_include', array(), 'array' );
			...
			$packageid=$post['packageid'];
			$season_options = JArrayHelper::getValue( $post, 'off_season', array(), 'array' );
...
*/
				...
				if ($isNew=="yes")
				{
					$obj = new stdClass();
					$obj->packageid = (int)$packageid;
					$obj->off_season   = $season_options[$i];
					$obj->two   = $code;
					$obj->three_five   =  $three_options[$i];
					$obj->six_nice   =  $six_options[$i];
					//$obj->price_include   =  $price_options[$i];
	...
	switch ($this->_task)
		{
			case 'apply':
				$msg = JText::_( 'Changes to  saved' );
					if($post["package_class"]=="small"){
					$link = 'index.php?option=com_cntours&view=cntours&c=price&task=editsmprice&cid[]='.$post['packageid'];
					}else{
					$link = 'index.php?option=com_cntours&view=cntours&c=price&task=editprice&cid[]='.$post['packageid'];					
					}
				break;
	...
	function remove()
	{
		...
		if($packclass=="small"){
		$this->setRedirect( "index.php?option=com_cntours&view=cntours&c=price&task=editsmprice&cid[]=$packageid", $msg );}else{
		$this->setRedirect( "index.php?option=com_cntours&view=cntours&c=price&task=editprice&cid[]=$packageid", $msg );		
		}
	}
