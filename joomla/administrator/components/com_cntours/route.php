 ...
 * @subpackage	Config
 */
class CntoursControllerRoute extends JController
{
	/**
	 * Custom Constructor
	 */
...
//["textfieldcheck"]=> string(0) "" ["c293f03092a989911df312e31cbf88f2"]=> string(1) "1" }
	...
	$post['package_tips'] = JRequest::getVar( 'package_tips', '', 'post', 'string', JREQUEST_ALLOWRAW );		
		...
		if($_FILES['ufile']['size']){
			...
			$thum=new resizeimage(JPATH_SITE.DS."uploads".DS."chinaTours".DS.$return_obj->_fileList['name'],200,155,0,JPATH_SITE.DS."uploads".DS."chinaTours".DS."thumb".DS."thumb_".$return_obj->_fileList['name']);
			$row->filedir=null;
			$row->ufile=null;
		...
		if((int)$post['hotel_grade']==0){
			$row->hotel_grade_str="";
			$row->hotel_grade=0;
			$post['hotel_name']="";
			$post['hotel_grade_str']="";
		}elseif(!$post['hotel_name']){
			$row->hotel_grade_str="";
			$row->hotel_grade=0;
			$post['hotel_grade']=0;
			$post['hotel_grade_str']="";
		...
		switch ($this->_task)
		{
			case 'apply':
				...
				$link = 'index.php?option=com_cntours&view=cntours&task=edit&cid[]='. $row->packageid;
				break;

			case 'save':
			default:
				$msg = JText::_( 'saved' );
				$link = 'index.php?option=com_cntours&view=cntours&c=route&task=editroute&cid[]='.$row->packageid;
				break;
		}
