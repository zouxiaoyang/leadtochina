...
function ajax_recommand(id,recommend,obj){
jq.post('index.php?option=com_hotel&c=upload&view=uploadpic&task=recommand',{id:id,recommend:recommend},function(ret){
	if(ret==1){
		//alert(jq(obj).attr('href'));
		if(jq(obj).html()=="推荐"){
			jq(obj).html('已推荐');
			}else if(jq(obj).html()=="已推荐"){
				jq(obj).html('推荐');
				}
		alert("设置成功！");
		}else{
