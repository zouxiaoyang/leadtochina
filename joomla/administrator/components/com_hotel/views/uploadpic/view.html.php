 ...
 * @since 1.0
 */
class HotelViewUploadpic extends JView
{
...
*/
		$db->setQuery("SELECT id,pic_dir,recommend ,pic_path FROM jos_cos_hotelpic  WHERE hotelid=$uid ORDER BY id DESC");
		//$pro_res=$db->query();
		...
		for($i=1;$i<=$pic_count;$i++){
			...
			if($piclist[$temp_index]->recommend==1){
				$recommend="已推荐";
			}else{
				$recommend="推荐";
			}
			$pic_str.="<td><img src=\"$temp_file_path\"><br /><a href=\"#\" onclick=\"javascript:del('../index.php?option=com_delpic&from=hotel&id={$piclist[$temp_index]->id}')\">删除</a> <a href=\"javascript:;\" onClick=\"ajax_recommand('{$piclist[$temp_index]->id}','{$piclist[$temp_index]->recommend}',this);\">$recommend</a></td>";
