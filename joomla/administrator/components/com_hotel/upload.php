 ...
 * @subpackage	Config
 */
class HotelControllerUpload extends JController
{
	/**
	 * Custom Constructor
	 */
...
//["textfieldcheck"]=> string(0) "" ["c293f03092a989911df312e31cbf88f2"]=> string(1) "1" }
		...
		switch ($this->_task)
		{
			case 'apply':
				...
				$link = 'index.php?option=com_hotel&view=cntours&task=edit&cid[]='. $row->hotelid;
				break;

			case 'save':
			default:
				$msg = JText::_( 'saved' );
				$link = "index.php?option=com_hotel&c=upload&view=uploadpic&task=upload&cid[]=". $post['hotelid'];
				break;
		}
...
*/
	...
	function recommand(){
		...
		$recommend=JRequest::getvar("recommend");
		if($recommend==1){
			$recommend_val=0;
		}else{
			$recommend_val=1;
		}
		$db->setQuery("update jos_cos_hotelpic set recommend='$recommend_val' where id='$id'");
		if($db->query()){
			echo 1;
			exit;
		}else{
