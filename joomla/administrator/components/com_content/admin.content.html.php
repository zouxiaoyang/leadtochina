		...
		<form action="index.php?option=com_content" method="post" name="adminForm">
			...
			for ($i=0, $n=count( $rows ); $i < $n; $i++)
			{
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td>
					<a href="index.php?option=com_uploadpic&c=upload&view=uploadpic&task=upload&cid[]=<?php echo $row->id;?>">上传图片</a>
					</td>
					<?php
					if ( $times ) {
						?>
   ...
   function updatePage() {
     if (request.readyState == 4) {
       if (request.status == 200) {
		...
		<?php echo JHTML::_( 'form.token' ); ?>
		</form><script> window.onload=SetCity;</script>
		<?php
	...
	*/
	...
	function _displayArticleDetails(&$row, &$lists )
	{
		...
		while ($pro_row=mysql_fetch_assoc($pro_res)){
			if(empty($row)==false and $row->provinceid==$pro_row['id']){
