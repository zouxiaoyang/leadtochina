...
class ContentController extends JController
{
	...
	*/
	function saveContent()
	{
		...
		$post1=JRequest::get('post');

		if($post1['provinceid']>0){
		
			$db->setQuery("SELECT name FROM jos_cos_province WHERE id={$post1['provinceid']}");
			$pro_res=$db->query();
			$pro_row=mysql_fetch_array($pro_res);
			$post1['province_name']=$pro_row['name'];
			$row->province_name=$post1['province_name'];
		...
		if($post1['cityid']>0){
			...
			$db->setQuery("SELECT name FROM jos_cos_city WHERE id={$post1['cityid']}");
			$city_res=$db->query();
			$city_row=mysql_fetch_array($city_res);
			$post1['city_name']=$city_row['name'];
			$row->city_name=$post1['city_name'];
