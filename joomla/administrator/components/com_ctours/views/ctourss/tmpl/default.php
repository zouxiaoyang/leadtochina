...
//假如是China Small Groups类型的话。换另一个模板显示
		...
		$link1     = JRoute::_( 'index.php?option=com_ctours&view=cntours&task=edit&cid[]='. $row->id );
	...
	?>
		<tr class="<?php echo "row$k"; ?>">
            ...
            <td align="center">
            <?php  if($row->category_parentid==77):?>
                <a style="color:#f00;font-weight:bold;" href="<?php echo 'index.php?option=com_tourhighlights&view=tourhighlights&cid='. $row->id?>">查看所有</a>
            <?php endif;?>
            </td>
			<td align="center">
				<a href="<?php echo 'index.php?option=com_ctours&view=cntours&c=route&task=editroute&cid[]='. $row->id?>">编辑行程</a>
			</td>
			<td align="center">
				<a href="<?php echo "index.php?option=com_ctours&c=price&view=price&task=editprice&cid[]=". $row->id?>">编辑价格</a>				
			</td>
			<td align="center">
			<?php 	
			$pic_num=$db->getOne("select count(id) from jos_cos_uploadpic where packageid='$row->id'");		
			?>
				<a href="<?php echo "index.php?option=com_ctours&c=upload&view=uploadpic&task=upload&cid[]=". $row->id?>">上传图片(<?php echo $pic_num;?>)</a>				
			</td>
			<td align="center">
			<?php 
			$db=&JFactory::getDbo();
			...
			$total=$db->loadResult();
			?>
				<a href="<?php echo "index.php?option=com_testimonisals&packageid=". $row->id."&catid=".$row->category_parentid."&chinatours=2";?>">Testimonisals管理(<?php echo $total;?>)</a>				
			</td>
			
			<td align="center">
				<?php echo $row->id; ?>
                                <?php echo "<br/>".$row->category_parentid;?>
			</td>
		</tr>
		<?php
			$k = 1 - $k;
...
<script>
	function ajax_views_change(id,obj){
		...
		jq.post("index.php?option=com_ctours",{task:'update_views',id:id, views:jq(obj).val()},function(){
