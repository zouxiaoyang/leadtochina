 ...
 * @since 1.0
 */
class SceneryViewScenery extends JView
{
	function display($tpl = null)
	{
		...
		$poll =& JTable::getInstance('scenery', 'Table');
...
*/
		...
		if(empty($options[0]->categoreid)==false){
			
			$db->setQuery("SELECT parent_id,sce_cat_path,title FROM jos_categories WHERE id={$options[0]->categoreid}");
			$pcat_res=$db->query();
		...
		$db->setQuery("SELECT id,title FROM jos_categories WHERE parent_id=0 AND section='com_scenery' AND published=1");
		$cat_res=$db->query();
		$cat_seled="";
		$cat_str='';
