 ...
 * @subpackage	Config
 */
class SceneryControllerUpload extends JController
{
	/**
	 * Custom Constructor
	 */
...
//["textfieldcheck"]=> string(0) "" ["c293f03092a989911df312e31cbf88f2"]=> string(1) "1" }
		...
		if($_FILES['ufile']['size']){
			...
			$fupload=new Upload(JPATH_ROOT.DS."uploads".DS."scenery","sce","com_scenery");
			...
			$thum=new resizeimage(JPATH_SITE.DS."uploads".DS."scenery".DS.$return_obj->_fileList['name'],140,140,0,JPATH_SITE.DS."uploads".DS."scenery".DS."thumb".DS."thumb_".$return_obj->_fileList['name']);

			$thum=new resizeimage(JPATH_SITE.DS."uploads".DS."scenery".DS.$return_obj->_fileList['name'],500,315,1,JPATH_SITE.DS."uploads".DS."scenery".DS.$return_obj->_fileList['name']);
			$row->pic_dir=null;
			$row->pic_path=null;
		...
		switch ($this->_task)
		{
			...
			default:
				$msg = JText::_( 'saved' );
				$link = "index.php?option=com_scenery&c=upload&view=uploadpic&task=upload&cid[]=". $post['sceneryid'];
				break;
		}
...
*/
	function cancel()
	{
		...
		$db		=& JFactory::getDBO();
		$row	=& JTable::getInstance('scenery', 'Table');

		$row->checkin( $id );
		$this->setRedirect( 'index.php?option=com_scenery' );
	}
}