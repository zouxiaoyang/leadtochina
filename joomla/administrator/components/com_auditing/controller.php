 ...
 * @subpackage	Config
 */
class AuditingController extends JController
{
	/**
	 * Custom Constructor
	 */
	...
	function display( )
	{
		switch($this->getTask())
		{
			...
			case 'edit'    :
			{
				...
				JRequest::setVar( 'view', 'auditing'  );//文件路径设置。是view
		...
		if(empty($view)) {
			JRequest::setVar('view', 'auditings');
		};
...
if(empty($post['service'])==false){
	$post['service'] = JRequest::getVar( 'service', '', 'post', 'string', JREQUEST_ALLOWRAW );	
		...
		switch ($this->_task)
		{
			case 'apply':
				$msg = JText::_( 'saved' );
				$link = 'index.php?option=com_auditing&view=hotel&task=edit&cid[]='. $row->id .'';
				break;

			case 'save':
			default:
				$msg = JText::_( 'saved' );
				$link = 'index.php?option=com_auditing';
				break;
		}
	...
	function remove()
	{
		...
		{
			$poll =& JTable::getInstance('auditing', 'Table');
			if (!$poll->delete( $cid[$i] ))
			{
				$msg .= $poll->getError();
			}
		}
		$this->setRedirect( 'index.php?option=com_auditing', $msg );
	}
	...
	*/
	...
	function publish()
	{
		...
		if (count( $cid ) == 1)
		{
			$row =& JTable::getInstance('auditing', 'Table');
			$row->checkin( $cid[0] );
		}
		$mainframe->redirect( 'index.php?option=com_auditing' );
	}
	function unpublish()
	{
		// Check for request forgeries
...
$db 	=& JFactory::getDBO();
		...
		$this->setRedirect( 'index.php?option=com_auditing' );
	}
	function cancel()
	{
		// Check for request forgeries
