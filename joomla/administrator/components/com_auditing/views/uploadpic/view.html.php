...
*/
		$db->setQuery("SELECT id,pic_dir ,pic_path FROM jos_cos_hotelpic  WHERE hotelid=$uid ORDER BY id DESC");
		//$pro_res=$db->query();
		...
		for($i=1;$i<=$pic_count;$i++){
			...
			$pic_str.="<td><img src=\"$temp_file_path\"><br /><a href=\"#\" onclick=\"javascript:del('../index.php?option=com_delpic&from=hotel&id={$piclist[$temp_index]->id}')\">删除</a></td>";
