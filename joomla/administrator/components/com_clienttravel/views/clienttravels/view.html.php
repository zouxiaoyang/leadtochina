 ...
 * @since 1.0
 */
class ClienttravelViewClienttravels extends JView
{
	function display( $tpl = null )
	{
		...
		if ($search)
		{
			$where[] = 'LOWER(m.package_name) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
		}
		...
		. ' FROM #__cos_client_package  AS m'
		. $where
		;
		$db->setQuery( $query );
		$total = $db->loadResult();
