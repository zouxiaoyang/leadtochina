 ...
 * @subpackage	Config
 */
class ClienttravelControllerUpload extends JController
{
	/**
	 * Custom Constructor
	 */
...
//["textfieldcheck"]=> string(0) "" ["c293f03092a989911df312e31cbf88f2"]=> string(1) "1" }
		...
		if($_FILES['ufile']['size']){
			...
			$thum=new resizeimage(JPATH_SITE.DS."uploads".DS."chinaTours".DS.$return_obj->_fileList['name'],200,200,0,JPATH_SITE.DS."uploads".DS."chinaTours".DS."thumb".DS."thumb_".$return_obj->_fileList['name']);
			$row->pic_dir=null;
			$row->pic_path=null;
		...
		switch ($this->_task)
		{
			...
			default:
				$msg = JText::_( 'saved' );
				$link = "index.php?option=com_clienttravel&c=upload&view=uploadpic&task=upload&cid[]=". $post['c_packageid'];
				break;
		}
