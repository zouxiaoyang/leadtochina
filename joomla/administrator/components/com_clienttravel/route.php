 ...
 * @subpackage	Config
 */
class ClienttravelControllerRoute extends JController
{
	/**
	 * Custom Constructor
	 */
...
//["textfieldcheck"]=> string(0) "" ["c293f03092a989911df312e31cbf88f2"]=> string(1) "1" }
	...
	$post['package_tips'] = JRequest::getVar( 'package_tips', '', 'post', 'string', JREQUEST_ALLOWRAW );		
		...
		if($_FILES['ufile']['size']){
			...
			$fupload=new Upload(JPATH_ROOT.DS."uploads".DS."chinaTours","cndes");
			...
			$thum=new resizeimage(JPATH_SITE.DS."uploads".DS."chinaTours".DS.$return_obj->_fileList['name'],200,150,0,JPATH_SITE.DS."uploads".DS."chinaTours".DS."thumb".DS."thumb_".$return_obj->_fileList['name']);
			$row->filedir=null;
			$row->ufile=null;
		...
		if($post['hotel_grade']>0){
			...
			if($post['hotel_grade']==136){
				...
				$hotel_grade_str="3 Star";
			}elseif($post['hotel_grade']==137){
			
			   $hotel_grade_str="4 Star";

			}elseif($post['hotel_grade']==138){
				$hotel_grade_str="5 Star";
			}
		...
		if($post['c_packageid']){
			$db->setQuery("SELECT package_name FROM jos_cos_client_package  WHERE id=".$post['c_packageid']);
			$p_res=$db->loadAssoc();
			$pname=$p_res['package_name'];
		...
		switch ($this->_task)
		{
			case 'apply':
				...
				$link = 'index.php?option=com_clienttravel&view=cntours&task=edit&cid[]='. $row->packageid;
				break;

			case 'save':
			default:
				$msg = JText::_( 'saved' );
				$link = 'index.php?option=com_clienttravel&view=clienttravel&c=route&task=editroute&cid[]='.$row->c_packageid;
				break;
		}
	...
	function remove()
	{
		...
		$packageid =JRequest::getVar( 'c_packageid', 0, '', 'int' );
		...
		$this->setRedirect( "index.php?option=com_clienttravel&view=clienttravel&c=route&task=editroute&cid[]=$packageid", $msg );
	}
...
*/
	function cancel()
	{
		...
		$id		= JRequest::getVar( 'c_packageid', 0, '', 'int' );
		...
		$this->setRedirect( "index.php?option=com_clienttravel&c=route&task=editroute&cid[]=$id" );
		}
	}
}