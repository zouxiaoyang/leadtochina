...
<?php
	...
	JToolBarHelper::title(  JText::_( '景点' ).': <small><small>[ ' . $text.' ]</small></small>' );
	JToolBarHelper::Preview('index.php?option=com_scenery&controller=scenery&cid[]='.$cid[0]);
	JToolBarHelper::save();
 ...
 document.getElementById('spancat3').style.display="none";
   ...
   function displayCat(val,selid) {
	...
	if(empty($this->sec_parentid)==false){
		...
		$temp="&fromsec=com_scenery&from=1&ccatid=".$this->sec_parentid;

		echo "var cidStr='$temp';";

	}else{
		echo "var cidStr='&fromsec=com_scenery&from=1';";
	}
	?>
	}
	...
	if(empty($this->third_parentid)==false){
		...
		$temp="&fromsec=com_scenery&from=2&ccatid=".$this->third_parentid;

		echo "var cidStr='$temp';";

	}else{
		echo "var cidStr='&fromsec=com_scenery&from=2';";
	}
	?>
	}
	...
	if(empty($this->four_parentid)==false){
		...
		$temp="&fromsec=com_scenery&from=3&ccatid=".$this->four_parentid;

		echo "var cidStr='$temp';";

	}else{
		echo "var cidStr='&fromsec=com_scenery&from=3';";
	}
	?>
	}
...
&nbsp;&nbsp;&nbsp;&nbsp;<span id="spancat3" style="display: none;"></span>
		...
		<tr>
			...
			<td>
			<?php 	echo $editor->display( 'notes',  htmlspecialchars($this->options['0']->notes,ENT_QUOTES), '550', '300', '60', '20', array('pagebreak', 'readmore'));?>
		...
		<tr>
			...
			<td>
			<?php 	echo $editor->display( 'remark',  htmlspecialchars($this->options['0']->remark,ENT_QUOTES), '550', '300', '60', '20', array('pagebreak', 'readmore'));?>
...
<script> window.onload=SetCity;
if(document.getElementById("cat1").options[document.getElementById("cat1").selectedIndex].value!=0){
