 ...
 * @subpackage	Config
 */
class CusorderController extends JController
{
	/**
	 * Custom Constructor
	 */
	...
	function display( )
	{
		switch($this->getTask())
		{
			...
			case 'showOder'    :
			{
				...
				JRequest::setVar( 'view', 'cusorder'  );//文件路径设置。是view
		...
		if(empty($view)) {
			JRequest::setVar('view', 'cusorders');
		};
	...
	function remove()
	{
		...
		{
			$poll =& JTable::getInstance('cusorder', 'Table');
			if (!$poll->delete( $cid[$i] ))
			{
				$msg .= $poll->getError();
			}
		}
		$this->setRedirect( 'index.php?option=com_cusorder', $msg );
	}
...
*/
	...
	{
		JRequest::checkToken() or jexit( 'Invalid Token' );
		$this->setRedirect( 'index.php?option=com_cusorder' );
	}
}