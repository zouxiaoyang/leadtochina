 ...
 * @subpackage	Config
 */
class FlicruorderController extends JController
{
	/**
	 * Custom Constructor
	 */
	...
	function display( )
	{
		...
		{			
			case 'showOder'    :
			{
				...
				JRequest::setVar( 'view', 'flicruorder'  );//文件路径设置。是view
		...
		if(empty($view)) {
			JRequest::setVar('view', 'flicruorders');
		};
	...
	function save()
	{
		...
		$query="UPDATE jos_cos_cruflight_order SET order_state=".$order_state." WHERE id=".$id;
		$db->setQuery( $query);
		//echo $query;
		//exit;
		if($db->query()){
		...
		$link = 'index.php?option=com_flicruorder';
	...
	function cancel()
	{
		...
		$this->setRedirect( 'index.php?option=com_flicruorder' );
	}
}