 ...
 * @since 1.5
 */
class CruiseModelcruiseroom extends JModel
{
	/**
	 * cruise id
	 *
...
//					' WHERE w.id = '.(int) $this->_id;
                        ...
                        $query ='SELECT * FROM #__cos_cruiseroom WHERE id='.(int) $this->_id;
   ...
   function store($data)
	...
	function delete($cid = array())
	{
		...
		if (count( $cid ))
		{
			...
			$cids = implode( ',', $cid );
			//echo $cids;die;
			$query = 'DELETE FROM #__cos_cruiseroom'
				. ' WHERE id IN ( '.$cids.' )';
			$this->_db->setQuery( $query );
			if(!$this->_db->query()) {
				$this->setError($this->_db->getErrorMsg());
