 ...
 * @subpackage	Config
 */
class CruiseControllerCruiseact extends JController
{
	/**
	 * Custom Constructor
	 */
	function __construct( $default = array())
	{
		...
		$this->registerTask( 'editcruiseact' , 		'display' );//编辑行程
	...
	function display( )
	{
		...
		switch($this->getTask())
		{
			case 'add'     :
			{
				...
				JRequest::setVar( 'view', 'cntoursact'  );
			...
			{
				JRequest::setVar( 'hidemainmenu', 1 );
				JRequest::setVar( 'layout', 'cruiseact'  );
				JRequest::setVar( 'view', 'cruiseact'  );
				JRequest::setVar( 'edit', false  );
			} break;
	...
	function save()
	{
		...
		$db		=& JFactory::getDBO();
		$model = $this->getModel('cruiseact');
		//print_r($model); 
		//$row	=& JTable::getInstance('cruiseact', 'Table');
		//print_r($post);exit;
		...
		$link ='index.php?option=com_cruise&view=cntours&c=cruiseact&task=cruiseact&cid[]='.$post['cruiseid'];
