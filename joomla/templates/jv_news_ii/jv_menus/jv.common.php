...
class MenuSystem{
        ...
        function _showMenuDetail($row, $level = 0) {
            ...
            if($row->id==186){
				$hot_img = '<b style="position:absolute; width:35px; text-align:right; z-index:1; float:left; margin-top:-45px; padding-left:65px;"><img src="/images/tours/01_03.png"  border="0"/></b>';
			}

            if ($row->url != null) {

                if ($row->browserNav == 0) {
					if($row->url=='/about-us/' OR $row->url=='/contact-us/'){
						 $menuItem = '<a href="'.$row->url.'" rel="nofollow" '.$Class.' '.$id.' '.$title.'>'.$str.$hot_img.'</a>';
					}else{
						
						$menuItem = '<a href="'.$row->url.'" '.$Class.' '.$id.' '.$title.'>'.$str.$hot_img.'</a>';
					}
