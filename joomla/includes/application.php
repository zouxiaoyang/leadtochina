...
class JSite extends JApplication
{
	...
	*/
	function render()
	{
		...
		if($option=="com_diy"){
			...
			$basetmpl=JPATH_DEFTMPL;
		}else{
			$basetmpl=JPATH_THEMES;
		}
		...
		switch($format)
		{
			...
			default     :
			{
				...
				if (!is_dir( $basetmpl.DS.$template ) && !$this->getCfg('offline')) {
					$file = 'component';
				}
				$params = array(
					'template' 	=> $template,
	...
	*/
	...
	function getTemplate()
	{
		...
		//echo $basetmpl.DS.$template.DS.'index.php';
		//exit;
		// Fallback template
		if (!file_exists($basetmpl.DS.$template.DS.'index.php')) {
