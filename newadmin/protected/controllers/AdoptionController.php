...
*/
  ...
  public function actionHomeLandProvince($id)
  {
    ...
    if(!$packageProvince[1]->seo_title_adoption){$this->setPageTitle($title . " " . $provinceDescription->name);}else{$this->setPageTitle($packageProvince[1]->seo_title_adoption);}
   if(!$packageProvince[1]->seo_keyword_adoption){ Yii::app()->clientScript->registerMetaTag('keywords','');}else{Yii::app()->clientScript->registerMetaTag('keywords',$packageProvince[1]->seo_keyword_adoption);}
    if(!$packageProvince[1]->seo_description_adoption){ Yii::app()->clientScript->registerMetaTag('description', $title .' Free to customize private China adoption travel now.');}else{Yii::app()->clientScript->registerMetaTag('description',$packageProvince[1]->seo_description_adoption);}
