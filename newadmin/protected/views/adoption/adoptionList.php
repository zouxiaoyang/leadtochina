...
$this->setPageTitle($article->title);
  //Yii::app()->clientScript->registerMetaTag('keywords',"China Adoption Travel Guide, Children Reunion Tour, China Adoption Travel Information, China adoption situation");
  Yii::app()->clientScript->registerMetaTag('description',$article->title);
...
<script>
$(function(){
	$("#adoption_left_category_"+<?php echo $article->cat_id;?>).show();
	$(".guidecont").find(".bigdq").click(function(){
	   $("li[id^=adoption_left_category_]").hide();
       $(this).next("li").toggle();
    })
})
function adoptionLeftCategory(obj){
	$(obj).show();
}
</script>
